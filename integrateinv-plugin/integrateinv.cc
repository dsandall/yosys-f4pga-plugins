...
struct Pin {
    ...
    // Yosys 0.55+ uses hash_into(Hasher) instead of unsigned int hash()
        ...
        for (auto it : m_Inverters) {
            ...
            if (sinks == pins) {
                ...
                for (auto pin : pins) {
                    ...
                    // In Yosys 0.55+, RTLIL::Const::operator[] returns by value,
                    // so we must modify the underlying bits vector directly.
                    if (invMask.bits().at(pin.bit) == RTLIL::State::S0) {
                        invMask.bits().at(pin.bit) = RTLIL::State::S1;
                    } else if (invMask.bits().at(pin.bit) == RTLIL::State::S1) {
                        invMask.bits().at(pin.bit) = RTLIL::State::S0;
                    } else {
